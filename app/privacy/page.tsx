...
export default function PrivacyPage() {
  return <LegalPage title="Privacy Policy">
    ...
    <h2>Processors</h2><p>Configured hosting, PostgreSQL, email, Redis, and Google Gemini providers process data to operate the service. When you use chat or study tools, your prompts and relevant extracted material are sent to the Gemini API. When you upload a file that does not plainly read as study material, an excerpt of its text and your stated topic are also sent to the Gemini API to check that it is suitable for study; clearly academic documents are checked without leaving MindForge. MindForge currently uses Gemini&apos;s free tier, where Google states submitted content may be used to improve its products. Read-aloud sends the text of the professor reply you select to the Gemini speech API, which returns generated audio; MindForge asks Google not to retain that request. The returned audio is cached in your account&apos;s records so repeat playback does not regenerate it, and it is deleted when you delete the message, session, or account.</p>
